...
class Paysera {
  ...
  generatePaymentUrl(options) {
    if (!options.amount || !options.currency || !options.firstName || !options.lastName || !options.email) {
      throw new Error('Paysera: Missing required payment fields.');
    }
  ...
  _signData(data) {
    ...
    return crypto.createHash('md5').update(`${dataString}|${this.signPassword}`).digest('hex');
  }
}
